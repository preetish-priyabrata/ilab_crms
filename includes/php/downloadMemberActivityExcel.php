...
while ($row = mysqli_fetch_array($res)) {
    if ($i == 2) {
        $sqlEmp = "select empl_name from tbl_employee where empl_id=" . $row[25];
    ...
    if ($_SESSION['contViewSetting'] == "No" && $empId != $row['empl_id']) {
    ...
    $phpExcel->getActiveSheet()->setCellValue("Q$i", $rowEmp['empl_name']);
